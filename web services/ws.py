...
def create_personaje():
    ...
    if conn:
        try:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO personajes (id, name, email, whatsapp) VALUES (%d, %s, %s, %d)", (id, name, email, whatsapp))
            conn.commit()
...
def update_personaje(id):
    ...
    if conn:
        try:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE personajes SET name = %s, email = %s, whatsapp = %d WHERE id = %d", (name, email, whatsapp, id))
            conn.commit()
